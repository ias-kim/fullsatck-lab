...
document.addEventListener('DOMContentLoaded', () => {
  // 주요 DOM 요소 할당
  const typingSection = document.querySelector('.typing-effects');
  ...
  const tilltEffectSection = document.querySelector('.tilt-effects-container');
  const tiltContents = document.querySelector('.tilt-effects');
  ...
  function registTiltEffectHandler(contents) {
    tilltEffectSection.addEventListener('mousemove', (event) => {
      const { left, top, width, height } =
        tilltEffectSection.getBoundingClientRect();
